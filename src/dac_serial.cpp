...
void divideAndConquer2(const std::vector<std::vector<int>>& X,
  ...
  // if possible perform basic merge phase of merge sort
  if(k0+1 == d){ // **********************************************************
  ...
  if(k0+1 < d){
    ...
    divideAndConquer2(X, D, thresh, ids, l0, l0+split0, l1+split1, r1, k0+1, true);
  }
}
...
void divideAndConquer(const std::vector<std::vector<int>>& X,
  ...
  // Rcpp::Rcout << "AAA ids (l to r): " <<  std::endl;
  // for(int i=l; i<r; i++){
  //   int x = ids[i];
  //   Rcpp::Rcout << x << " ";
  // } 
  // Rcpp::Rcout <<  std::endl;
  // Rcpp::Rcout << "X (l to r): " <<  std::endl;
  // for(int i=l; i<r; i++){
  //   int x = X[ids[i]][1];
  //   Rcpp::Rcout << x << " ";
  // } 
  ...
  // Rcpp::Rcout << "ids (l to r): " <<  std::endl;
  // for(int i=l; i<r; i++){
  //   int x = ids[i];
  //   Rcpp::Rcout << x << " ";
  // } 
  ...
  // } 
  // Rcpp::Rcout <<  std::endl;
  // Rcpp::Rcout << "idss (l to r): " <<  std::endl;
  // for(int i=0; i<r-l; i++){
  //   int x = ids_sorted[i];
  //   Rcpp::Rcout << x << " ";
  // } 
  // Rcpp::Rcout <<  std::endl;
  // Rcpp::Rcout << "Xs (l to r): " <<  std::endl;
  // for(int i=0; i<r-l; i++){
  //   int x = X[ids_sorted[i]][1];
  //   Rcpp::Rcout << x << " ";
  // } 
  // Rcpp::Rcout <<  std::endl;
  // Rcpp::Rcout <<  std::endl;
...
// [[Rcpp::export]]
IntegerMatrix dac_sssserial(NumericMatrix X, int thresh = 100, bool brute_force = false) {
  int n = X.nrow();
  int d = X.ncol();

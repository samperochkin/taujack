...
// [[Rcpp::export]]
IntegerMatrix dac_seqq(IntegerMatrix X, int thresh = 25, bool brute_force = false) {
  int n = X.nrow();
  int d = X.ncol();
  ...
  for(int i=0; i<n; i++) ids[X_vec[i][0]] = i;
  std::vector<int> idss = ids;
  
  if(brute_force){
    conquer(X_vec, D, ids, 0, n, 1);
  }else{
    divideAndConquer(X_vec, D, thresh, ids, idss, 0, n, 1);
  }

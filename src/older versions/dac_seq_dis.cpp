...
std::pair<int, int> findSplit(const std::vector<std::vector<int>>& X,
  ...
  while (count0 < n0 && count1 < n1) {
    ...
    // temp_nss = count1 * (n0 - count0);
    temp_nss = count0 * (n1 - count1);
...
// [[Rcpp::export]]
IntegerMatrix dac_seq_dis(NumericMatrix X, int thresh = 25, bool brute_force = false) {
  int n = X.nrow();
  int d = X.ncol();

...
bss <- lapply(1:p, \(k){
  data[row_ids[,k],] %>% filter(day == 1, month == 1, year %in% seq(1840+23,2024-23,23)) %>% select(time) %>% unlist
})
...
ggplot(data0, aes(x=year, y=m_mt, col=station_name, group=station_name)) +
  theme_light() +
  geom_line(data = data0, aes(y=m_mu), alpha=.75) +
  geom_point()

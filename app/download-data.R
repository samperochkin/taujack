
###########################################################################
# Script dowloading data from Weather Canada ------------------------------
###########################################################################



# packages ----------------------------------------------------------------

# install.packages("weathercan",  repos = c("https://ropensci.r-universe.dev", "https://cloud.r-project.org"))
# install.packages(c('lutz', 'sf')) # dependencies
library(weathercan)
library(dplyr)



# select stations ---------------------------------------------------------

?stations
names(stations())
stations() %>%
  filter(start <= 1900, end >= 2000, interval == "day") %>%
  print(n=nrow(.))

# Interesting selections:
# station_id %in% c(271, 568, 707, 2925, 4333, 4712, 5051)
# airports: !is.na(TC_id)

stns <- stations() %>%
  filter(start <= 1900, end >= 2000, interval == "day",
         station_id %in% c(271, 568, 707, 2925, 4333, 4712, 5051))
# stns <- stations() %>%
#   filter(start <= 1900, end >= 2000, interval == "day", !is.na(TC_id))

stns_id <- stns %>%
  dplyr::select(station_id) %>%
  unlist(use.names = F)




# download raw data -------------------------------------------------------

# from terminal:
# R CMD BATCH --no-restore app/download-data.R app/log_dd.txt

for(stn in stns_id){
  # start_end <- stns %>% filter(station_id == stn) %>% select(start, end)
  # start_end <- as.Date(c(paste0(start_end[1], "-01-01"), paste0(start_end[2], "-12-31")))
  # start <- stns %>% filter(station_id == stn) %>% select(start)
  # start <- as.Date(paste0(start, "-01-01"))
  
  # somehow we have to specify start and end (bug in the current implementation of weather_dl)
  data <- weather_dl(stn, interval = "day", trim = F, verbose = T)
  # data <- weather_dl(stn, interval = "day", start = start_end[1], end = start_end[2], trim = F, verbose = T)
  # data <- weather_dl(stn, interval = "day", time_disp = "UTC", trim = F, verbose = T)
  # data <- weather_dl(stn, interval = "day", start = start, trim = F, verbose = T)
  saveRDS(data, paste0("app/data/daily_data_", stn, ".rds"))
}

# record successful downloads
stns_id <- sapply(strsplit(list.files("app/data/", "daily_data"), "_"), \(x) x[3])
stns_id <- as.numeric(sapply(strsplit(stns_id, "\\."), \(x) x[1]))
saveRDS(stns_id, "app/data/stns_id.rds")

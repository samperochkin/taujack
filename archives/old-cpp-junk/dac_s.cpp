  ...
  for (auto it1 = pos1_map.begin(); it1 != pos1_map.end(); ++it1) {
    ...
    for(auto it2 = pos2_map.begin(); it2 != pos2_map.end(); ++it2) {
      ...
      if(v_g > 0){ // blocks necessarily discordant 
        Rcpp::Rcout << "hey " << ks[0] << std::endl;
        for (int k = 0; k < d; k++) {
          if (v[k] == 0 && ks[k] == 0) {
            for(int r=k+1; r<d; r++){
              if(ks[r]-ks[r-1] > 1){
                for(int i : ids1i) D[i][ks[r-1]] += n2j;
                for(int j : ids2j) D[j][ks[r-1]] += n1i;
                break;
              }
              else if(ks[r]-ks[r-1] == 1 && v[r] == 1){
                for(int i : ids1i) D[i][ks[r]-1] += n2j;
                for(int j : ids2j) D[j][ks[r]-1] += n1i;
                break;
              }
            ...
            for(int i : ids1i) D[i][ks[k]-1] += n2j;
            for(int j : ids2j) D[j][ks[k]-1] += n1i;
          }  
        }
        continue;
      }
...
void divideAndConquer(const std::vector<std::vector<double>>& X,
  ...
  for (auto it1 = pos_map.begin(); it1 != pos_map.end(); ++it1) {
    ...
    divideAndConquer(X, C, D, thresh, ids1);
    ...
    for (auto it2 = std::next(it1); it2 != pos_map.end(); ++it2) {
      ...
      if((v_g > 0) && (v_e + v_g < d)){ // blocks that are necessarily discordant
        // for (int k = 0; k < d; k++) {
        //   if (pos1[k] > pos2[k]) {
        //     for(int i : ids1) D[i][0] += n2;
        //     for(int j : ids2) D[j][0] += n1;
        //     break;
        //   }
        // }
        continue;
        ...
        divideAndConquerLower(X, C, D, thresh, ids1, ids2, ks);
      } 
    } 
  } 
} 
...
// [[Rcpp::export]]
IntegerVector dac_s(NumericMatrix X, int thresh = 100, bool brute_force = false, bool ret_C = true) {
  int n = X.nrow();
  int d = X.ncol();
  
  // initialize C
  std::vector<int> C(n, 0);
  std::vector<std::vector<int>> D(n, std::vector<int>(d-1));
  ...
  if(brute_force){
    conquer(X_vec, C, D, ids);
  }else{
    divideAndConquer(X_vec, C, D, thresh, ids);
  }
  
  return Rcpp::wrap(C);
  // convert C and return it
  // if(ret_C) return Rcpp::wrap(C);
  // std::vector<int> D_sum(n, 0);
  // for(int i=0; i<n; i++){
  //   for(int k=0; i<d-1; k++) D_sum[i] += D[i][k];
  //   for(int k=0; i<d-1; k++) D_sum[i] = n - 1 - D_sum[i];
  // }
  // return Rcpp::wrap(D_sum);
}
